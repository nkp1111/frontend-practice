    ...
    <li className='border-t border-custom-dark mt-16 text-custom-dark py-5 group selection:text-custom-light selection:bg-custom-dark'>
      <Link href={item.link}>
        <div className='flex gap-5 flex-col sm:flex-row'>
          <div className='relative overflow-hidden flex-shrink-0 flex-grow sm:flex-grow-0'>
            <Image
              src={item.image}
              alt={item.title}
              width={200}
              height={200}
              className='w-full h-64 sm:w-64 sm:h-48 lg:w-96 lg:h-64 object-center object-cover transition-all duration-300 ease-linear group-hover:scale-105'
            />
          </div>
          <div className='flex flex-col justify-between'>
            <div>
              <h3 className='font-medium text-3xl lg:text-5xl lg:w-10/12 group-hover:underline transition-all duration-300 ease-linear'>{item.title}</h3>
              <p className='text-sm'>{item.description.slice(0, 40)}</p>
            </div>
            <p className='uppercase sm:block hidden'>Press {item.press}</p>
          </div>
          <div className='relative flex sm:justify-start-start justify-between items-center'>
            <p className='uppercase block sm:hidden'>Press {item.press}</p>
