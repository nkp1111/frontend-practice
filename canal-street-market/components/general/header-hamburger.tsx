...
export default function HeaderHamburger({ setActivePage }: HamburgerNavParams) {
  const [showHamburger, setShowHamburger] = useState(true);
  return (
    <div className={`fixed top-0 left-0 w-full h-screen pb-5 overflow-y-auto flex flex-col transition-custom-transition`}>

      <div className={`w-10 h-10 cursor-pointer absolute top-8 right-10 transition-custom-transition`}
        onClick={() => setShowHamburger(pre => !pre)}>
        {showHamburger ? <HamburgerMenu /> : <CloseIcon />}
      </div>

      <div className={`flex flex-col gap-5 ${showHamburger ? "hidden" : "block"}`}>
        {/* grid nav  */}
